...
import { ChevronDown, Folder as FolderIcon, MoreVertical, Star, Edit2, FolderInput, Trash2, FolderPlus } from "lucide-react";
      ...
      {!collapsed && (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
          {displayFolders.map((item) => (
            <div
              key={item.id}
              onClick={() => onOpenFolder(item.raw)}
              className="group relative flex items-center justify-between p-3 rounded-2xl bg-[#f0f4f9] hover:bg-[#e1e5ea] border border-transparent hover:border-[#c4c7c5] transition-all cursor-pointer shadow-sm"
            >
              <div className="flex items-center gap-3 min-w-0 pr-1">
                ...
                <div className="min-w-0">
                  ...
                  <span className="text-[11px] text-[#444746] truncate block">{item.location}</span>
                </div>
              </div>
