...
async def get_corpus_completeness(db: AsyncSession, tenant_id: UUID, corpus_folder_id: UUID) -> Dict[str, Any]:
    ...
    (
        ...
        data_loss_words_total, documents_with_data_loss,
    ) = doc_res.one()
...
async def get_completeness_drill(db: AsyncSession, tenant_id: UUID, corpus_folder_id: UUID, category: str) -> List[Dict[str, Any]]:
    ...
    raise HTTPException(status_code=400, detail=f"Unknown drill category '{category}'. Valid: missing_fields, failed_pages, data_loss_documents, unverified_facts, machine_facts")
