...
Renumbered 0046 -> 0050 during a merge of `scanner-feature` into
`feature-kunal-DMS`: this migration and 0046_restricted_app_role (the
D-2 RLS fix) had both branched from 0045_seed_starter_templates
independently. Content unchanged from the original commit -- only the
revision id, down_revision, and filename moved to keep the migration
chain linear instead of introducing a second head. Genuinely valuable,
independent work: closes exactly the tenant_id/RLS gap on
doc_dg_metadata_items this session's own T05 work (0047_metadata_item_regions)
had explicitly left out of scope.

Revision ID: 0050_enable_rls_missing_tables
Revises: 0049_marathi_translations
...
revision: str = '0050_enable_rls_missing_tables'
down_revision: Union[str, None] = '0049_marathi_translations'
